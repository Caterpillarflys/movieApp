<template>
    <div class="wrap">
        <div class="list" v-for="(item,index) in comingList" :key="index">
            <div class="img">
            ...
            <div class="message">
                <div class="message-list">
                    ...
                    <p class="text-color longLimit">{{item.star?("主演"+item.star):''}}</p>
                    <p class="p-last text-color longLimit">
                        {{item.rt+"上映"}}
                    </p>
                </div>
                <div :class="item.showst=4?'buy':'sell'">
                    {{item.showst=4?"想看":"预售"}}
                </div>
            </div>
        </div>
...
<script>
import {getMovieComming} from 'api/movie'
export default {
    name:'movieComming',
    async created(){
...
;color:#fff}
.buy{width: 94px;height: 54px;border-radius: 4px;background: #faaf00;
font-size: 24px;line-height: 54px;white-space: nowrap;text-align: center
;color:#fff}
